...
1. Load history in parallel: `habitron` `get_habit_history`, `get_task_history`, and `get_journal_history` for the window. If the window is 30 days, also pull `get_habit_history` for 90 days so you can tell a blip from a trend.
...
3. Answer in this shape, briefly:
   ...
   - **One suggestion** — smaller than I'd expect (shrink, move, or pair a habit; never add). Offer to apply it (e.g. archive, change days) only if I agree.
4. If I confirm a stable insight, store it with `add_memory` (category `obstacle` or `preference`).
