...
1. **Load context first.** Call `habitron` `get_day_context` for the date, `get_day_review_history` (7 days) and `list_tags` before saying anything. The context returns local `now`, so use that to know whether the day is already underway. If calendar, Linear, or email tools are available, pull that day's fixed commitments too — meetings and deadlines are non-negotiable constraints.
2. **Intake before output.** Don't draft yet. Ask at most 2–3 concise questions the context doesn't already answer, typically: what matters most, what is fixed. Skip anything the data answers.
2b. **Capacity is already on record — don't ask for it cold.** `get_day_review_history` (7 days) carries yesterday's `energy` and `calm`. Apply the axis mapping in `COACH-CLAUDE.md` to it. Say what you read and what you are doing about it in one line ("energy was a 2 last night, so this is a lighter day — say if that's wrong"), rather than asking me to rate myself again. Only ask outright when there is no recent review to read.
