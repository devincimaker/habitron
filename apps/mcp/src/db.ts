...
export async function getHabitsByIds(ids: string[]): Promise<Map<string, Habit>> {
  if (ids.length === 0) return new Map();
  const rows = unwrap(
...
export async function listHabitLogs(start: string, end: string): Promise<HabitLogRecord[]> {
  const rows = unwrap(
    await supabase
      .from('habit_logs')
...
export async function addMemory(content: string, category: MemoryCategory): Promise<Memory> {
  const row = unwrap(
    await supabase
      .from('memories')
...
export async function getActivePlan(date: string): Promise<DailyPlan | null> {
  const row = unwrap(
    await supabase
      .from('daily_plans')
      .select('*')
...
export async function listPlans(start: string, end: string): Promise<DailyPlan[]> {
  const rows = unwrap(
    await supabase
      .from('daily_plans')
      .select('*')
...
export async function saveAcceptedPlan(input: {
  date: string;
  rationale?: string;
  items: PlanItemInput[];
}): Promise<DailyPlan> {
...
export async function setPlanItemOutcome(
  itemId: string,
  outcome: DailyPlanItemOutcome
